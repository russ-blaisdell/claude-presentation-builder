...
def generate_readme(yaml_path, proof_dir, output_path):
    ...
    lines.append("")
    lines.append("| Token | Generic Default | Customizable via |")
    lines.append("|-------|----------------|------------------|")
    lines.append("| Heading font | Arial | `brand.yaml` `fonts.heading` |")
    lines.append("| Body font | Arial | `brand.yaml` `fonts.body` |")
    lines.append("| Primary | `#1A365D` (navy) | `brand.yaml` `colors.primary` |")
    lines.append("| Secondary | `#3182CE` (blue) | `brand.yaml` `colors.secondary` |")
    lines.append("| Accent | `#63B3ED` | `brand.yaml` `colors.accent` |")
    lines.append("| Light background | `#EBF4FF` | `brand.yaml` `colors.background_light` |")
    lines.append("| Canvas | 10\" x 5.625\" (16:9) | Template-driven |")
    lines.append("| Theme | 12 color slots + 2 font slots (OOXML) | `theme.json` |")
    lines.append("")
