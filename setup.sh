...
# Write REPO_PATH so the skill can find this builder regardless of cwd
REPO_PATH_FILE="$HOME/.claude/skills/create-presentation-config"
mkdir -p "$(dirname "$REPO_PATH_FILE")"
cat > "$REPO_PATH_FILE" <<EOF
# Auto-generated by setup.sh — used by the /create-presentation skill
# to locate the builder repo regardless of the user's current directory.
# Last setup: $(date)
REPO_PATH=$SCRIPT_DIR
PYTHON=$VENV_DIR/bin/python3
EOF
ok "Builder path written ($REPO_PATH_FILE)"
